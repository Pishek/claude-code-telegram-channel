...
3. **Маркер сессии в чате.** Когда новая сессия захватывает бота, в чат уходит
   ...
   тег генерится самим плагином, а папка берётся из `CLAUDE_PROJECT_DIR` —
   каталога, откуда запущен `claude` (не из рабочего каталога самого плагина).
4. **Хештег сессии на каждом сообщении.** Этот же `#<тег>` ставится шапкой на
   каждый ответ Claude и на permission-запросы (`🔐 Permission: <tool> #<тег>`),
   так что поиском по `#<тег>` в Telegram находится вся переписка сессии. Тег
   всегда содержит букву — иначе Telegram не делает хештег кликабельным/искомым.
...
## Обновление плагина (свои правки)

Версия плагина в Claude Code привязана к коммиту репо, поэтому правки в `tg/`
надо сначала закоммитить, а потом протолкнуть через marketplace-кеш:

```
# правишь файлы в tg/ → git commit (SHA берётся из HEAD локального репо)
/plugin marketplace update cc-channels    # CC перечитывает папку, видит новый коммит
/plugin update tg@cc-channels             # тянет новую версию в кеш
/reload-plugins                           # применить (--force, если меняются MCP-тулзы канала)
```
