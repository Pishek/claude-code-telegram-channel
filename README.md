...
Две правки в обработке permission-запросов (полный diff —
...
2. **Ответ одним словом.** Помимо строгой формы `yes <id>`, голое
   `yes` / `no` / `да` / `нет` (регистр не важен, хвостовая пунктуация диктовки
   вроде `Да.` допускается) применяется к последнему ожидающему запросу. Именно
...
- **Одним словом** `yes` / `no` / `да` / `нет` — применяется к последнему
  ожидающему запросу. На Apple Watch: Reply на уведомлении → продиктовать.
